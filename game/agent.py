...
class MLPQNetwork:
    ...
    def forward(self, x: np.ndarray) -> tuple[list[np.ndarray], list[np.ndarray]]:
        ...
        for index, (weight, bias) in enumerate(zip(self.weights, self.biases)):
            ...
            current = np.maximum(z, 0.0) if index < len(self.weights) - 1 else z
...
class SnakeAgent:
    ...
    def train_step(self) -> float | None:
        ...
        self.stats.epsilon = max(self.config.EPSILON_MIN, self.stats.epsilon * self.config.EPSILON_DECAY)
